...
namespace Shelly.GraphQLShared.Services
{
     public class HttpGraphQLClientService : IHttpGraphQLClientService
     {
          ...
          public async Task<GenericResponse<Response>> Get<Response, Request>(GraphQLRequest data)
          {
               ...
               SetAuthenticationHttpClient<Request>(data);
               var response = await ExecRequest<GenericResponse<Response>, GenericResponse<Response>>($"", HttpMethodTypes.GET);
               if (response.Status)
          ...
          public async Task<GenericResponse<Response>> Post<Response, Request>(GraphQLRequest data)
          {
               CreateRequest<Request>(data);
               SetAuthenticationHttpClient<Request>(data);
               var response = await ExecRequest<GenericResponse<Response>, GenericResponse<Response>>($"", HttpMethodTypes.POST);
               if (response.Status)
                    return response.Data;
               return response.Errors;
          }

          private void SetAuthenticationHttpClient<Request>(GraphQLRequest data)
          {
               if (data is GraphQLTokenRequest)
               {
                    _httpClient.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", ((GraphQLTokenRequest)data).Token);
               }
          }
          // helper methods
          ...
          {
               string uri;
               if (String.IsNullOrEmpty (uriPathMethod))
                    uri = $"{_httpClient.BaseAddress}";
               else
                    uri = $"{_httpClient.BaseAddress.ToString().PathURLFormat()}{uriPathMethod}";
               switch (httpMethod)
               {
